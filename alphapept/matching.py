...
__all__ = ['calculate_distance', 'calib_table', 'align', 'calculate_deltas', 'calculate_deltas_spark',
           'calculate_distance_parallelized', 'align_files', 'align_datasets', 'get_probability', 'convert_decoy',
           'match_datasets', 'match_datasets_spark', 'match_data']
...
# Cell
import alphapept.spark

def calculate_deltas_spark(settings: dict, filenames: list, calib:bool = False, callback:Callable=None) -> (pd.DataFrame, np.ndarray, dict):
    ...
    Args:
        settings (dict): Dictionary containing specifications of the run
        combos (list): A list containing tuples of filenames that should be compared.
        calib (bool): Boolean flag to indicate distance should be calculated on calibrated data.
        callback (Callable): A callback function to track progress.
    ...
    """
    spark = alphapept.spark.create_spark_session(settings)
    offset_dict = {}
    dfs = []

    logging.info("Reading hdf files into memory")
    for filename in filenames:
        hdf_filename = os.path.splitext(filename)[0] + '.ms_data.hdf'
        df = alphapept.io.MS_Data_File(hdf_filename).read(dataset_name="peptide_fdr").set_index('precursor')
        if not offset_dict:
            offset_dict = {'mz':'relative', 'rt':'absolute'}
            if 'mobility' in df.columns:
                logging.info("Also using mobility for calibration.")
                offset_dict['mobility'] = 'relative'
            cols = offset_dict.keys()
            deltas = pd.DataFrame(columns = cols)
        dfs.append(df[cols].to_dict())

    #broadcast file data (https://sparkbyexamples.com/pyspark/pyspark-broadcast-variables/)
    dfs_broadcast = spark.sparkContext.broadcast(dfs)

    #parallelize the list of file pairs
    combos = list(combinations(range(len(filenames)), 2))
    rdd = spark.sparkContext.parallelize(combos)

    #execute the parallelized process and collect results
    results = rdd.map(lambda x: calculate_distance_parallelized(x, dfs_broadcast, offset_dict, calib)).collect()

    indexes, dists, weights = list(zip(*results))
    combos = [(filenames[i[0]], filenames[i[1]]) for i in indexes]
    deltas = pd.DataFrame(dists, columns = cols, index=combos)
...
def calculate_distance_parallelized(indexes: tuple, dfs_broadcast: pyspark.broadcast.Broadcast, offset_dict: dict, calib: bool = False) -> (list,int):
    ...
    Args:
        indexes (tuple): Pair of file indices
        dfs_broadcast (pyspark.broadcast.Broadcast): A broadcasted list of file data in dictionary form
        offset_dict (dict): Dictionary with column names and how the distance should be calculated.
        calib (bool): Boolean flag to indicate distance should be calculated on calibrated data.
        callback (Callable): A callback function to track progress.
    ...
    Returns:
        pd.DataFrame: Dataframe containing the deltas of the files
        np.ndarray: Numpy array containing the weights of each comparison (i.e. number of shared elements)
        dict: Offset dictionary which was used for comparing.

    '''
    dfs = dfs_broadcast.value
    df_1 = pd.DataFrame(dfs[indexes[0]])
    df_1.index.name = 'precursor'
    df_2 = pd.DataFrame(dfs[indexes[1]])
    df_2.index.name = 'precursor'
    dists, weights = calculate_distance(df_1, df_2, offset_dict, calib)
    return indexes, dists, weights
...
def align_datasets(settings:dict, callback:callable=None):
    ...
    if len(filenames) > 1:
        ...
        if settings['matching'].get('spark'):
            deltas, weights, offset_dict = calculate_deltas_spark(settings, filenames, callback=cb)
        else:
...
def match_datasets_spark(settings:dict, callback:Callable = None):
    """Match datasets: Wrapper function to match datasets based on a settings file, using Spark.
    This implementation uses matching groups but not fractions.
    ...
    if len(settings['experiment']['file_paths']) > 2:
        ...
        for group in set(settings['experiment']['matching_group']):
            ...
            if len(files_from) > 2:
                ...
                spark = alphapept.spark.create_spark_session(settings)
                #Parallelize over every file
                rdd = spark.sparkContext.parallelize(files_to)

                #No return value
                _ = rdd.map(lambda x: match_data(x, grouped, lookup_dict, shortnames_lookup, std_, std_range,
                    alignment_cols, use_mobility, extra_cols, match_d_min, match_p_min)).collect()
...
def match_data(file_to: str, grouped: pd.DataFrame, lookup_dict: dict, shortnames_lookup: dict, std_: pd.DataFrame, std_range: np.array,
        alignment_cols: list, use_mobility: bool, extra_cols: list, match_d_min: float, match_p_min: float):
    '''
    Mapped function extrapolated from code in match_datasets
